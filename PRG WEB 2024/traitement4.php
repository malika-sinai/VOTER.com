...
function tableau(){
   $res = '<h2 id= "no_table">Vous n avez pas de vote en cours actuellement.</h2>';
   $scrutin = recupScrutin();
   if(empty($scrutin)){
      $res = $res.'<div align = "center"><input type = button value = "OK" id = "ok"></div>';
      return $res;
   }else{
     $res = '<h2 id = titre> la liste de vos votes en cours </h2>
        <div id = "table_vote" align = "center">
        <table id = "liste_scrutin" border = 1 align = center>
        <tr>
        <th> Titre </th>
        <th> Organlisateur </th>
        <th>statut</th>
      ...
      foreach($scrutin as $iter){
        $res = $res.'<tr>
                      <td id = "titre">'. $iter["titre"].'</td>
                      <td id = "organisateur">'. $iter["organisateur"].'</td>';
        if($iter["voter"] == "oui"){
